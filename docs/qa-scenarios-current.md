...
| 영속 초기화 (resetState) | ✅ 구현 | Dashboard 헤더 우측 `초기화` 버튼 (`ghost-button`). 클릭 시 `window.confirm` 으로 한 번 더 확인 후 `actions.resetState()` 호출 |
...
- 시작 전 Dashboard 헤더 우측 `초기화` 버튼으로 localStorage 비움 (confirm 다이얼로그 경유)
...
- **Given**: 모달 열림 → `일정` 탭 선택
- **Then**: 입력 필드 — 제목 / 내용 / **날짜** (`<input type="date">`)
- **When**: 제목·내용·날짜 모두 입력 후 `생성 완료`
- **Then**: 콘텐츠 type=`schedule`, `date` 필드 포함되어 저장. `ContentList` 의 "작성된 일정" 섹션에 노출 (`MM.DD` 형식)
- **Validation**: 날짜 미입력 시 `생성 완료` 버튼 disabled (메모 탭에서는 날짜 무관)
...
- **Given**: 임의 진행 상태
- **When**: Dashboard 헤더 우측 `초기화` 클릭 → `window.confirm` 다이얼로그에서 확인
- **Then**:
  - localStorage 에서 키 삭제
  - 새로고침 없이 즉시 초기 상태로 복귀
  - `activeRecommendation`, `classifications`, `usedFeatures`, `dismissedGuides` 모두 초기화
  - 새 작성 흐름이 정상 동작
- **취소 케이스**: confirm 다이얼로그에서 취소 → 상태 변경 없음
